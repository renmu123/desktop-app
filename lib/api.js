...
var Api = {
	...
	auth: function(email, pwd, host, callback) { 
		...
		// console.log('????????????')
		needle.post(this.getUrl('auth/login', {macAddr: macAddr, v: currentVersion}), {email: email, pwd: pwd}, {timeout: 10000, rejectUnauthorized: false}, function(error, response) {
	...
	logout: function (callback) {
		needle.post(this.getUrl('auth/logout', {macAddr: macAddr, v: currentVersion}), {timeout: 10000, rejectUnauthorized: false}, function(error, response) {
			callback();
		});
	},
	...
	getSyncNotebooks: function(afterUsn, maxEntry, callback) {
		...
		// console.log(url);
		needle.get(url, {rejectUnauthorized: false},
	...
	getSyncNotes: function(afterUsn, maxEntry, callback) {
		...
		needle.get(url, {rejectUnauthorized: false}, function(error, response) {
			...
			var ret = response.body;
			console.log("getSyncNotes(): ret=", ret);
			if(Common.isOk(ret)) {
				callback && callback(ret);
			} else {
				callback && callback(false);
	...
	getLastSyncState: function(callback) {
		...
		// console.log(url);
		needle.get(url, {timeout: 10000, rejectUnauthorized: false}, function(error, response) {
	...
	addNotebook: function(notebook, callback) {
		...
		Notebook.getServerNotebookIdByNotebookId(notebook.ParentNotebookId, function(serverNotebookId) {
			...
			needle.post(me.getUrl('notebook/addNotebook'), data, {rejectUnauthorized: false}, function(err, resp) {
				me.checkError(err, resp);
				if(err) {
	...
	updateNotebook: function(notebook, callback) {
		...
		Notebook.getServerNotebookIdByNotebookId(notebook.ParentNotebookId, function(serverNotebookId) {
			...
			log('update notebook');
			log(data);
			needle.post(me.getUrl('notebook/updateNotebook'), data, {rejectUnauthorized: false}, function(err, resp) {
				me.checkError(err, resp);
				if(err) {
					log('err');
					log(err);
	...
	deleteNotebook: function(notebook, callback) {
		...
		log('delete notebook');
		needle.post(me.getUrl('notebook/deleteNotebook'), data, {timeout: 10000, rejectUnauthorized: false}, function(err, resp) {
			me.checkError(err, resp);
			if(err) {
				return callback(false);
			}
	...
	getNote: function(noteId, callback) {
		var me = this;
		needle.get(me.getUrl('note/getNote', {noteId: noteId}), {rejectUnauthorized: false}, function(error, response) {
			me.checkError(error, response);
			if(error) {
				return callback && callback(false);
			}
	...
	updateNote: function(note, callback) {
		...
		Notebook.getServerNotebookIdByNotebookId(note.NotebookId, function(serverNotebookId) {
			...
			needle.post(me.getUrl('note/updateNote'), data, {multipart: needMultiple, rejectUnauthorized: false}, function(err, resp) {
	...
	deleteTrash: function(note, callback) {
		...
		log('delete note');
		// 这里要重新require下, 不然为{}
		Note = require(path.resolve(appPath, './lib/note'));
		needle.post(me.getUrl('note/deleteTrash'), data, {timeout: 10000, rejectUnauthorized: false}, function(err, resp) {
			me.checkError(err, resp);
			if(err) {
				return callback(false);
			}
	...
	exportPdf: function(noteId, callback) {
		...
		needle.get(me.getUrl('note/exportPdf', {noteId: noteId}), {rejectUnauthorized: false}, function(err, resp) {
			me.checkError(err, resp);
			if(err) {
				return callback && callback(false);
			}
	...
	addTag: function(title, callback) {
		var me = this;
		needle.post(me.getUrl('tag/addTag'), {tag: title}, {rejectUnauthorized: false}, function(err, resp) {
			me.checkError(err, resp);
			if(err) {
				return callback && callback(false);
			}
	...
	deleteTag: function(tag, callback) {
		var me = this;
		needle.post(me.getUrl('tag/deleteTag'), {tag: tag.Tag, usn: tag.Usn}, {rejectUnauthorized: false}, function(err, resp) {
			me.checkError(err, resp);
			if(err) {
				return callback && callback(false);
			}
	...
	getImageTest: function(callback) {
		needle.get('http://localhost:9000/images/logo.png', {rejectUnauthorized: false}, function(err, resp) {
			// log(resp.body);
			/*
			{ 'accept-ranges': 'bytes',
			  'content-disposition': 'inline; filename="logo.png"',
	...
	uploadImage: function() {
		...
		needle.post('http://localhost/phpinfo.php', data, { multipart: true ,rejectUnauthorized: false }, function(err, resp, body) {
